...
def main(_argv):
    ...
    image_list = open(Path(os.path.join(
            './data/voc2012_raw/VOCdevkit/VOC2012/', 'ImageSets', 'Main', '%s.txt' % FLAGS.split)).as_posix()).read().splitlines()
    logging.info("Image list loaded: %d", len(image_list))
    for name in tqdm.tqdm(image_list):
        annotation_xml = os.path.join(Path(
            './data/voc2012_raw/VOCdevkit/VOC2012/', 'Annotations', name + '.xml').as_posix())
