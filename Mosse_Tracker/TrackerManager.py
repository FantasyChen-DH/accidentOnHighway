...
from Mosse_Tracker.KCF import KCFTracker
from Mosse_Tracker.Mosse import MOSSE
from Mosse_Tracker.utils import RectSelector


from data.CONSTANTS import Work_Tracker_Interpolation
...
class Tracker:
    def __init__(self, frame, cut_size, frame_width, frame_height, tracker_id =0, tracker_type = TrackerType.MOSSE, label = 'car'):
        self.history = []
        self.label = label
        ...
        if tracker_type == TrackerType.MOSSE:
            ...
            self.tracker = MOSSE(frame_gray, cut_size,learning_rate=0.225,psrGoodness=5)
            self.addHistory(self.tracker.getCutFramePosition())

        elif tracker_type == TrackerType.KCF:

            self.tracker = KCFTracker(True, True, True)
            self.tracker.init([xmin, ymin, xmax - xmin, ymax - ymin], frame)
            ...
            self.tracker.start_track(frame_gray, dlib.rectangle(int(xmin), int(ymin), int(xmax), int(ymax)))
            self.addHistory([xmin, ymin, xmax, ymax])
            self.dx = []
            self.dy = []
    ...
    def update(self, frame):
        if self.tracker_type == TrackerType.MOSSE:
            ...
            self.tracker.updateTracking(frame_gray,is_stopped)
            self.addHistory(self.tracker.getCutFramePosition())

        elif self.tracker_type == TrackerType.KCF:
            boundingbox = self.tracker.update(frame)
            cut_size = boundingbox[0], boundingbox[1], boundingbox[0]+boundingbox[2], boundingbox[1]+boundingbox[3];
            if len(self.dx) == 0:
                self.dx.append(0)
                self.dy.append(0)
            else:
                x,y = self.get_position(cut_size)
                xold, yold = self.get_position(self.history[-1])
                dx, dy = x - xold, y - yold
                self.dx.append(dx)
                self.dy.append(dy)
            self.addHistory(cut_size)
    ...
    def isUnderSpeedLimit(self,from_frame_no = -1,to_frame_no = -1):
        if self.avg_speed[to_frame_no] == None:
            self.avg_speed[to_frame_no] = self.getAvgSpeed(from_frame_no, to_frame_no)
        if self.avg_speed[to_frame_no] < 10:
            return True
        return False
